...
// function to get the Index of address
...
// function used to find polluted block in cache 
PBLK replace_polluted(struct cache_blk_t *head){
    PBLK blk=NULL;
    PBLK repl_blk=NULL;
    for (blk=head;blk;blk=blk->way_next)
        {
          if(blk->status==0) //check whether block is valid
           return blk;
          if (blk->polluted == 1)
            repl_blk=blk;
        }
  ...
  repl=NULL;  
    if(isl3==1){
     if((cp->misses) % 5000 == 0){
       ...
       for(i=0;i<5000;i++){
         bypass[i]=0; 
       } // on every 5000th miss set bypass buffer of 5000 elements to 0
